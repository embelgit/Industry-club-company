...
import {getImport_Export} from '../../../service/RegistrationModule/ImportExportDetail';

const ImportExportDetailList = () => {
  ...
  const fetchTargetClintLocation = async (companyId, pageNo = 0) => {
    try {
      setLoader(true);
      const result = await getImport_Export(companyId, pageNo);
          ...
          <tbody>
            {filteredData.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  <Link
                    to={`/update-Import-Export-Detail/${index}`}
                    state={{importExportDetails: filteredData[index]}}
...
export default ImportExportDetailList;
